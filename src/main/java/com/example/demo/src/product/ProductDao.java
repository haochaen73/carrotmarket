...
import com.example.demo.src.product.model.PatchPriceReq;
...
public class ProductDao {
    ...
    public int createProduct(PostProductReq postProductReq) {
        String createProductQuery = "insert into Product (userIdx, categoryIdx, productName, price, productDetail, productMainImg) VALUES (?,?,?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createProductParams = new Object[]{postProductReq.getUserIdx(), postProductReq.getCategoryIdx(), postProductReq.getProductName(),postProductReq.getPrice(), postProductReq.getProductDetail(), postProductReq.getProductMainImg()}; // 동적 쿼리의 ?부분에 주입될 값
        this.jdbcTemplate.update(createProductQuery, createProductParams);
        // 즉 DB의 Product Table에 (userIdx, categroyIdx, productName, price, productDetail, productMainImg)값을 가지는 유저 데이터를 삽입(생성)한다.
    ...
    public int modifyProductName(PatchProductNameReq patchProductNameReq) {
        String modifyProductNameQuery = "update Product set productName = ? where productIdx = ? "; // 해당 productIdx를 만족하는 Product를 해당 productName으로 변경한다.
        Object[] modifyProductNameParams = new Object[]{patchProductNameReq.getProductName(), patchProductNameReq.getProductIdx()}; // 주입될 값들(nickname, userIdx) 순

        return this.jdbcTemplate.update(modifyProductNameQuery, modifyProductNameParams); // 대응시켜 매핑시켜 쿼리 요청(생성했으면 1, 실패했으면 0)
    }

    public int modifyPrice(PatchPriceReq patchPriceReq) {
        String modifyPriceQuery = "update Product set price = ? where productIdx = ? "; // 해당 productIdx를 만족하는 Product를 해당 productName으로 변경한다.
        Object[] modifyPriceParams = new Object[]{patchPriceReq.getPrice(), patchPriceReq.getProductIdx()}; // 주입될 값들(nickname, userIdx) 순
    ...
    public int modifyCategory(PatchCategoryReq patchCategoryReq) {
        String modifyCategoryQuery = "update Product set categoryIdx = ? where productIdx = ? "; // 해당 productIdx를 만족하는 Product를 해당 productName으로 변경한다.
        Object[] modifyCategoryParams = new Object[]{patchCategoryReq.getCategoryIdx(), patchCategoryReq.getProductIdx()}; // 주입될 값들(nickname, userIdx) 순

        return this.jdbcTemplate.update(modifyCategoryQuery, modifyCategoryParams); // 대응시켜 매핑시켜 쿼리 요청(생성했으면 1, 실패했으면 0)
    }
    ...
    public List<GetProductRes> getProducts() {
        String getProductsQuery = "select * from Product"; //User 테이블에 존재하는 모든 회원들의 정보를 조회하는 쿼리
    ...
    public List<GetProductRes> getProductsByProductName(String productName) {
        String getProductsByProductNameQuery = "select * from Product where productName =?"; // 해당 이메일을 만족하는 유저를 조회하는 쿼리문
        String getProductsByProductNameParams = productName;
        return this.jdbcTemplate.query(getProductsByProductNameQuery,
                (rs, rowNum) -> new GetProductRes(
                        rs.getInt("productIdx"),
                        rs.getInt("userIdx"),
                        rs.getInt("categoryIdx"),
                        ...
                        rs.getInt("price"),
                        rs.getString("productDetail"),
                        rs.getString("productMainImg")), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
    ...
    public GetProductRes getProduct(int productIdx) {
        String getProductQuery = "select * from Product where productIdx = ?"; // 해당 userIdx를 만족하는 유저를 조회하는 쿼리문
