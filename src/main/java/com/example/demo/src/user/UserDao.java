...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (phoneNumber, password, nickName, address, profileImg) VALUES (?,?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getPhoneNumber(), postUserReq.getPassword(), postUserReq.getNickName(),postUserReq.getAddress(), postUserReq.getProfileImg()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public int checkPhoneNumber(String phoneNumber) {
        String checkPhoneNumberQuery = "select exists(select phoneNumber from User where phoneNumber = ?)"; // User Table에 해당 phoneNumber 값을 갖는 유저 정보가 존재하는가?
        String checkPhoneNumberParams = phoneNumber; // 해당(확인할) 이메일 값
        return this.jdbcTemplate.queryForObject(checkPhoneNumberQuery,
                int.class,
                checkPhoneNumberParams); // checkEmailQuery, checkEmailParams를 통해 가져온 값(intgud)을 반환한다. -> 쿼리문의 결과(존재하지 않음(False,0),존재함(True, 1))를 int형(0,1)으로 반환됩니다.
    }
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx, phoneNumber, password, nickName, address, profileImg from User where phoneNumber = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    public List<GetUserRes> getUsersByNickName(String nickName) {
        ...
        return this.jdbcTemplate.query(getUsersByNickNameQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getString("nickName"),
                        rs.getString("address"),
                        rs.getString("profileImg")), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
